...
class HistoriList extends StatelessWidget {
  ...
  edit(Histori user, BuildContext context) {
    showDialog(
      context: context,
  ...
  String getShortName(Histori user) {
    String shortName = "";
    if (!user.nameSity.isEmpty) {
      shortName = user.nameSity.substring(0, 1) + ".";
    }

    if (!user.firstX.isEmpty) {
      shortName = shortName + user.firstX.substring(0, 1);
    }
    return shortName;
  }
}